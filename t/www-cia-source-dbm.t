...
#1
BEGIN { use_ok('WWW::CIA::Source::DBM') }
...
#2
my $source = eval {
    WWW::CIA::Source::DBM->new({DBM => './t/test.dbm'});
};
ok( ! $source, q(Shouldn't be able to create DBM in read-only mode) );

#3
$source = WWW::CIA::Source::DBM->new({DBM => './t/test.dbm', Mode => 'readwrite'});
ok( $source, 'new() - returns something' );

#4
ok( $source->isa('WWW::CIA::Source::DBM'), 'new() - returns WWW::CIA::Source::DBM object' );
...
undef $source;
$source = eval {
    WWW::CIA::Source::DBM->new({DBM => './t/test.dbm', Mode => 'read'});
};
ok( $source, 'new() - open existing DBM readonly' );

#12
ok( $source->isa('WWW::CIA::Source::DBM'), 'new() - open existing DBM - returns WWW::CIA::Source::DBM object' );
