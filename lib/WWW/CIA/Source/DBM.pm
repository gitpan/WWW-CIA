...
@WWW::CIA::Source::DBM::ISA = ("WWW::CIA::Source");
...
sub new {
    ...
    unless (exists $opts->{DBM}) {
        croak("WWW::CIA::Source::DBM: No DBM file specified");
    }

    my $mode;
    if (exists $opts->{Mode} && $opts->{Mode} eq "readwrite") {
        tie %{$self->{DBM}}, "MLDBM", $opts->{DBM}, O_CREAT|O_RDWR, 0640 or croak "WWW::CIA::Source::DBM: Can't open DBM: $!";
    } elsif (-e $opts->{DBM}) {
        tie %{$self->{DBM}}, "MLDBM", $opts->{DBM}, O_RDONLY, 0440 or croak "WWW::CIA::Source::DBM: Can't open DBM: $!";
    } else {
        croak "WWW::CIA::Source::DBM: $opts->{DBM}: $!";
    }
...
WWW::CIA::Source::DBM - an interface to a DBM copy of the CIA World Factbook
...
WWW::CIA::Source::DBM is an interface to a pre-compiled DBM copy of the CIA
World Factbook.

The module can also be used to make the DBM file, taking data from
WWW::CIA::Parser (or WWW::CIA::Source::Web) and inserting it into a DBM.

A script to do this - www-cia-makedbm.pl - should be included in this
modules's distribution.

=head1 METHODS

Apart from C<new>, these methods are normally accessed via a WWW::CIA object.

=over 4

=item C<new(\%opts)>

This method creates a new WWW::CIA::Source::DBM object. It takes a hashref of
...
(WWW::CIA::Parser also creates four extra fields: "URL", "URL - Print",
...
WWW::CIA, WWW::CIA::Parser, WWW::CIA::Source::Web
